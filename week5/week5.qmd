...
```{r}
rdd_lm <- ggplot(data_binned, 
                 aes(x = avg_distance, y =avg_deforest, 
                     color = as.factor(protected)))+
  geom_point(alpha = .5)+
...
rdd_ols <- lm(
  deforest ~ # outcome
    ...
    slope_cat + road_cat + water_cat + soil_cat, # Covariates
  data = sim_data
)
...
global_rdd <- rdrobust(
  ...
  covs = global_samp %>%  select(slope_cat, road_cat,
                                 water_cat, soil_cat), # Controls
  c = 0, # Cutoff at zero 
  kernel = 'triangular'
  )
...
rdd_poly <- rdplot(
  ...
  x = sim_data$distance,
  c = 0,
  binselect = 'es', # optimal evenly-spaced method using spacings estimators.
...
run_country_rdd <- function(country_name) {
  ...
  rdd_model <- rdrobust(
  y = df_country$deforest, # Outcome
  x = df_country$distance, # Running variable
  covs = df_country %>%  select(slope_cat, road_cat,
                                 water_cat, soil_cat), # Controls
  c = 0, # Cutoff at zero,
  p = 1,
  kernel = 'triangular'
...
rdd_6country <- lapply(unique(sim_data$country),
                       run_country_rdd)
