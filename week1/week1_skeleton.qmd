...
class_data <- read_csv("https://raw.github.com/garberadamc/W26-Policy-Eval/main/course-materials/labs/data/W26_class_survey.csv") %>% 
  mutate(random_groups = sample(rep(c('control',
                                      'treatment'),
                                   each = n()/2 )))
...
balance_summary <- class_data %>% 
  gtsummary::tbl_summary(
    ...
    # Display mean and sd for all 
    statistic = list(all_continuous() ~ "{mean} ({sd})") # Parentheses on sd puts the sd in parenthesis 
  ) %>% 
  # Add p value
  add_p()
...
tab_source_note(
  source_note = 'Note: Data from Winter 2026 Class Survey'
)
```
...
tbl_2way <- lobsters %>% 
  ...
  summarize(
    ...
    .groups = 'drop') %>% 
  # Pivot wider to create column for each region
  pivot_wider(
    names_from = region,
    values_from = total_cpue
  ) %>% 
  arrange(year) %>% 
  # add row for total cpue
  adorn_totals('row') # Janitor command that appends a total row and or column to df (here we call row)
```
...
```{r}
tbl_3way <- lobsters %>% 
  # Calculate total cpue for each year/region/treatment
  group_by(year, region, treat) %>% 
  summarize(total_cpue = sum(cpue, na.rm = TRUE),
            .groups = 'drop') %>% 
  # Pivot to create a column for each trt/control group within each region
...
Trash burning continuing even though other methods of trash disposal were becoming available 
2. How were the treatment and control groups formed? 
Total area was divided into 44, 22 were split into 2 groups and competed against each other. 
3. Was the social competition strategy effective? 
It was effective
5. What was the primary metric for assessing the impact of the social competition?
Reduction in new evidence of trash burning. 
