...
```{r}
did_data_az_ca %>% 
  mutate(date_obj = mdy(date),
         week = floor_date(date_obj, unit = 'week')) %>% 
  filter(year == 2020, month(date_obj) %in% c(2,3,4)) %>% 
  group_by(week,city) %>% 
  summarize(mean_n02 = mean(daily_max_1_hour_no2_concentration, na.rm = TRUE), 
            .groups = 'drop') %>% 
  ggplot(aes(x = week, y = mean_n02, color = city))+
  geom_line(size = 1.5)+
  geom_point(size = 2.5)+
  # add ca start date line 
  geom_vline(xintercept = as.Date("2020-03-19"),
             linetype = 'dashed',
             size = 3,
             color = 'goldenrod')+
  # Add CA start date label
  annotate("label", x = as.Date("2020-03-20"),
           y = 35,
           label = 'CA Lockdown \n Starts',
           size = 3)+
  labs(title = 'Weekly No2 Trends in 2020: Los Angeles vs Phoenix',
       x = 'Date',
