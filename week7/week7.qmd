...
**RESPONSE:** 49 states over 23 years
...
scm_data <- data %>%
  ...
  synthetic_control(
    outcome = co2_power_pc_tons, # Outcome var
    unit = state, # Unit identifier 
    time = year, # Time Identifier
    i_unit = "CA", # Treated unit
    i_time = 2013, # Treated year (intervention = i)
    
    # Running scm for every other state we had 
    generate_placebos = TRUE) %>%
  
  # Define what chars synthetic might match 
  
  generate_predictor(  
    time_window = 2000:2012, # Pre treatment time window
    coal_share = mean(coal_share, na.rm = TRUE), # average share of electricity from coal 
    income_pc = mean(income_pc, na.rm = TRUE), # average per cap income
    elec_price = mean(elec_price, na.rm = TRUE), # ave electricity price
    co2_pre_mean = mean(co2_power_pc_tons, na.rm = TRUE)) %>% # avg pre treat outcome
  
  # Take predictor summaries from above and actually figure out how much weight to give each donor state 
  generate_weights(
    optimization_window = 2000:2012, # Period over which donor weights are optimized
    margin_ipop = 0.02, # how close is close enough for the optimizer, smaller = more precise 
    sigf_ipop = 7, # How many sig figs
    bound_ipop = 6) %>% 
  
  # construct synthetic ca by using optimized donor weights
  generate_control() 
```
...
**RESPONSE:** High variability but general negative trend. Difference is around zero pre treatment. 
...
**RESPONSE:** Yes, but it seems to be moving back into the placebo area
